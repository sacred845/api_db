    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
		...
		*/
		...
		/*
			...
			$update = $em->getRepository(Update::class)->find($params['id']);
			$ishaserror = false;
			if ($update && $update->getFiles()->count()) {
				foreach ($update->getFiles() as $file) {
					$proc = new QueuesProcess();
					$proc->setStatus(QueuesProcess::STATUS_INPROGRESS)
							->setStartedAt(new \DateTime('now'))
							->setTask($task)
							->setParams(['file_id' => $file->getId()]);
					$em->persist($proc);
					$em->flush($proc);
					$process = new Process(array('/usr/bin/php', 
								$this->getContainer()->get('kernel')->getRootDir().'/../bin/console', 
				...
				$event = new \App\AdminBundle\Dispatch\Events\BaseUpdateEvent($update, $em);
